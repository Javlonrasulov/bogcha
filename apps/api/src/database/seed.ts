...
import { DeepPartial, EntityTarget, ObjectLiteral, QueryDeepPartialEntity, Repository } from 'typeorm';
...
/** TypeORM DataSource — main() ichida initialize qilinadi. */
let ds = dataSource;

function getRepo<E extends ObjectLiteral>(entity: EntityTarget<E>): Repository<E> {
  return ds.getRepository(entity);
}

function entityDefaults<E extends ObjectLiteral>(
  repo: Repository<E>,
  data: DeepPartial<E>,
): DeepPartial<E> {
  const ts = new Date();
  const record = { ...data } as Record<string, unknown>;
  const { columns } = repo.metadata;

  if (record.id === undefined && columns.some((column) => column.propertyName === 'id')) {
    record.id = randomUUID();
  }
  if (record.createdAt === undefined && columns.some((column) => column.propertyName === 'createdAt')) {
    record.createdAt = ts;
  }
  if (record.updatedAt === undefined && columns.some((column) => column.propertyName === 'updatedAt')) {
    record.updatedAt = ts;
  }

  for (const column of columns) {
    if (record[column.propertyName] != null) continue;
  ...
  return record as DeepPartial<E>;
}

function createRow<E extends ObjectLiteral>(repo: Repository<E>, data: DeepPartial<E>): E {
  return repo.create(entityDefaults(repo, data));
}

async function insertMany<E extends ObjectLiteral>(
  entity: EntityTarget<E>,
  rows: QueryDeepPartialEntity<E>[],
  batchSize = 500,
) {
  if (rows.length === 0) return;
  const repository = getRepo(entity);
  for (let i = 0; i < rows.length; i += batchSize) {
    const entities = rows
      .slice(i, i + batchSize)
      .map((row) => repository.create(entityDefaults(repository, row as DeepPartial<E>)));
...
async function resetDemoData() {
  ...
  await ds.query(`TRUNCATE TABLE ${identifiers} CASCADE`);
}
...
async function seedPlans() {
  ...
  for (const definition of definitions) {
    ...
    const existing = await planRepo.findOne({ where: { tier: definition.tier } });
    const plan = existing
      ? await planRepo.save(Object.assign(existing, definition))
      : await planRepo.save(createRow(planRepo, definition));
...
async function seedTenant(planId: string) {
  const tenantRepo = getRepo(Tenant);
  const settingsRepo = getRepo(TenantSettings);
  const tenant = await tenantRepo.save(
    createRow(tenantRepo, {
...
async function seedPlatformUsers(tenantId: string) {
  ...
  const superAdmin = await getRepo(User).save(createRow(getRepo(User), {
      tenantId: null,
  ...
  const owner = await getRepo(User).save(createRow(getRepo(User), {
      tenantId,
...
async function seedBranches(tenantId: string) {
  ...
  const branches = [];
  for (const definition of definitions) {
    branches.push(await getRepo(Branch).save(createRow(getRepo(Branch), { tenantId, ...definition })));
  }
  return branches;
}
...
async function seedExpenseCategories(tenantId: string) {
  ...
  for (const definition of definitions) {
    const created = await getRepo(ExpenseCategory).save(createRow(getRepo(ExpenseCategory), { tenantId, ...definition }));
...
async function seedIncomeCategories(tenantId: string) {
  ...
  for (const definition of definitions) {
    const created = await getRepo(IncomeCategory).save(createRow(getRepo(IncomeCategory), { tenantId, ...definition }));
    categories.set(definition.kind, created.id);
  }
  return categories;
}
...
async function seedSuppliers(tenantId: string) {
  ...
  for (const definition of SUPPLIERS) {
    const created = await getRepo(Supplier).save(createRow(getRepo(Supplier), {
        tenantId,
...
async function seedProducts(tenantId: string, suppliers: SupplierState[]): Promise<ProductState[]> {
  ...
  for (const [index, name] of PRODUCT_CATEGORIES.entries()) {
    const created = await getRepo(ProductCategory).save(createRow(getRepo(ProductCategory), { tenantId, name, sortOrder: index }));
  ...
  for (const definition of PRODUCTS) {
    ...
    const created = await getRepo(Product).save(createRow(getRepo(Product), {
        tenantId,
...
async function seedSupplierPriceHistory(
  ...
  const rows: QueryDeepPartialEntity<SupplierPrice>[] = [];
  ...
  await insertMany(SupplierPrice, rows);
  return suppliers;
}
...
async function seedRecipes(tenantId: string, products: ProductState[]): Promise<RecipeState[]> {
  ...
  for (const definition of RECIPES) {
    ...
    const recipeRepo = getRepo(Recipe);
    const created = await recipeRepo.save(
      createRow(recipeRepo, {
        tenantId,
        name: definition.name,
...
async function seedMenu(
  ...
  const menu = await getRepo(Menu).save(createRow(getRepo(Menu), {
      tenantId,
      branchId,
  ...
  for (const day of WEEKLY_MENU) {
    for (const [mealType, recipeName] of [
      ...
      const menuSlotRepo = getRepo(MenuSlot);
      const menuSlotRecipeRepo = getRepo(MenuSlotRecipe);
      const slot = await menuSlotRepo.save(
        createRow(menuSlotRepo, {
          menuId: menu.id,
          weekday: day.weekday,
          mealType,
        }),
      );
      await menuSlotRecipeRepo.save(
        menuSlotRecipeRepo.create({
          menuSlotId: slot.id,
          recipeId: byName.get(recipeName)!.id,
        }),
      );
    }
  }
...
async function seedGroups(
  ...
  for (const definition of groupSeeds) {
    const created = await getRepo(Group).save(createRow(getRepo(Group), {
        tenantId,
        branchId,
        name: definition.name,
...
async function seedStaff(
  ...
  for (const definition of staffSeeds) {
    ...
    if (definition.login && definition.role) {
      const user = await getRepo(User).save(createRow(getRepo(User), {
          tenantId,
      ...
      await getRepo(UserBranch).save({ userId: user.id, branchId });

      if (definition.groupIndex !== undefined && groups[definition.groupIndex]) {
        await getRepo(GroupTeacher).save({
          userId: user.id,
    ...
    const staff = await getRepo(Staff).save(createRow(getRepo(Staff), {
        tenantId,
        branchId,
        userId,
  ...
  if (staffSeeds.length > 6) {
    await getRepo(Staff).save(createRow(getRepo(Staff), {
        tenantId,
        branchId,
        firstName: 'Barno',
        lastName: 'Rasulova',
...
async function seedChildren(
  ...
  const childRows: QueryDeepPartialEntity<Child>[] = [];
  const guardianRows: QueryDeepPartialEntity<Guardian>[] = [];
  ...
  await insertMany(Child, childRows, 500);
  await insertMany(Guardian, guardianRows, 500);
...
async function seedOperations(ctx: OperationsContext) {
  ...
  const movementRows: QueryDeepPartialEntity<StockMovement>[] = [];
  const batchRows: QueryDeepPartialEntity<AttendanceBatch>[] = [];
  const attendanceRows: QueryDeepPartialEntity<AttendanceRecord>[] = [];
  const nutritionDayRows: QueryDeepPartialEntity<NutritionDay>[] = [];
  const nutritionLineRows: QueryDeepPartialEntity<NutritionDayLine>[] = [];
  const expenseRows: QueryDeepPartialEntity<Expense>[] = [];
  const purchaseOrderRows: QueryDeepPartialEntity<PurchaseOrder>[] = [];
  const purchaseItemRows: QueryDeepPartialEntity<PurchaseOrderItem>[] = [];
  const supplierPriceRows: QueryDeepPartialEntity<SupplierPrice>[] = [];
  const staffAttendanceRows: QueryDeepPartialEntity<StaffAttendance>[] = [];
  ...
  function receiveStock(
    ...
    unitPrice: number,
    date: Date,
    meta: Partial<QueryDeepPartialEntity<StockMovement>>,
  ) {
  ...
  await insertMany(PurchaseOrder, purchaseOrderRows, 200);
  await insertMany(PurchaseOrderItem, purchaseItemRows, 1000);
  await insertMany(Expense, expenseRows, 500);
  await insertMany(NutritionDay, nutritionDayRows, 300);
  await insertMany(NutritionDayLine, nutritionLineRows, 1000);
  await insertMany(AttendanceBatch, batchRows, 500);
  await insertMany(AttendanceRecord, attendanceRows, 2000);
  await insertMany(StockMovement, movementRows, 2000);
  await insertMany(SupplierPrice, supplierPriceRows, 2000);
  await insertMany(StaffAttendance, staffAttendanceRows, 2000);

  // Joriy qoldiq snapshoti.
  const stockItemRows: QueryDeepPartialEntity<StockItem>[] = [];
  ...
  await insertMany(StockItem, stockItemRows, 500);
  ...
  for (const product of products) {
    ...
    await getRepo(Product).update(product.id, { unitCost: money(weighted / totalQuantity) });
  }

  for (const [supplierId, balance] of supplierBalances) {
    await getRepo(Supplier).update(supplierId, { balance });
  }
...
async function seedFinance(
  ...
  const invoiceRows: QueryDeepPartialEntity<Invoice>[] = [];
  const paymentRows: QueryDeepPartialEntity<Payment>[] = [];
  const allocationRows: QueryDeepPartialEntity<PaymentAllocation>[] = [];
  const incomeRows: QueryDeepPartialEntity<Income>[] = [];
  const expenseRows: QueryDeepPartialEntity<Expense>[] = [];
  ...
  await insertMany(Invoice, invoiceRows, 1000);
  await insertMany(Payment, paymentRows, 1000);
  await insertMany(PaymentAllocation, allocationRows, 1000);
  await insertMany(Income, incomeRows, 1000);
  await insertMany(Expense, expenseRows, 1000);
...
async function seedPayroll(
  ...
  for (const periodStart of [addMonths(today, -2), addMonths(today, -1), monthStart(today)]) {
    ...
    for (const branch of branches) {
      ...
      const items: QueryDeepPartialEntity<PayrollItem>[] = [];
      let totalGross = 0;
      let totalNet = 0;
      let totalTax = 0;
      ...
      await getRepo(Payroll).save(createRow(getRepo(Payroll), {
          id: payrollId,
          tenantId,
          branchId: branch.id,
          period,
      ...
      await insertMany(PayrollItem, items.map((item) => ({ ...item, payrollId })));

      // Ish haqi xarajati faqat to'langan oy uchun yoziladi.
      if (!isCurrent) {
        await getRepo(Expense).save(createRow(getRepo(Expense), {
            tenantId,
            branchId: branch.id,
...
async function seedBudgets(
  ...
  for (const periodStart of [addMonths(today, -2), addMonths(today, -1), monthStart(today)]) {
    ...
    for (const branch of branches) {
      ...
      {
        const budgetRepo = getRepo(Budget);
        const budget = await budgetRepo.save(
          createRow(budgetRepo, {
            tenantId: tenant.id,
            branchId: branch.id,
            period,
        ...
        await insertMany(
          ...
          [
              ...
              ].map((line) => ({ ...line, budgetId: budget.id })),
        );
      }
    }
  }
...
async function seedNotificationsAndAudit(args: {
  ...
  const lowStock = await getRepo(StockItem)
    .createQueryBuilder('si')
    .leftJoinAndSelect('si.product', 'product')
    .leftJoinAndSelect('si.branch', 'branch')
  ...
  const debtorsRaw = await getRepo(Invoice)
    .createQueryBuilder('i')
    .select('COALESCE(SUM(i.balance), 0)', 'balance')
    .addSelect('COUNT(*)', 'count')
    .where('i.tenantId = :tenantId', { tenantId })
    .andWhere('i.status IN (:...statuses)', {
      statuses: [InvoiceStatus.OVERDUE, InvoiceStatus.PARTIALLY_PAID],
    })
    .getRawOne<{ balance: string; count: string }>();
  const debtors = {
    _sum: { balance: debtorsRaw?.balance ?? 0 },
    _count: Number(debtorsRaw?.count ?? 0),
  };

  const pendingOrders = await getRepo(PurchaseOrder)
    .createQueryBuilder('po')
    .leftJoinAndSelect('po.branch', 'branch')
    .where('po.tenantId = :tenantId', { tenantId })
    .andWhere('po.status = :status', { status: PurchaseOrderStatus.PENDING_APPROVAL })
    .getMany();
  ...
  for (const notification of notifications) {
    ...
    const recipientRepo = getRepo(NotificationRecipient);
    const saved = await notificationRepo.save(
      createRow(notificationRepo, { tenantId, ...rest, createdAt } as DeepPartial<Notification>),
    );
    await recipientRepo.insert(
      uniqueRecipients.map((userId, index) => ({
  ...
  const sampleNutritionDay = await getRepo(NutritionDay)
    .createQueryBuilder('nd')
    .leftJoinAndSelect('nd.lines', 'lines')
    .leftJoinAndSelect('lines.product', 'product')
    .where('nd.tenantId = :tenantId', { tenantId })
    .orderBy('nd.date', 'DESC')
    .take(1)
    .getOne();

  const auditRows: QueryDeepPartialEntity<AuditLog>[] = [
    {
      tenantId,
      userId: ownerId,
  ...
  await insertMany(AuditLog, auditRows);
}
...
async function printSummary(tenantId: string) {
  const [children, groups, staff, attendance, movements, invoices, payments, expenses, incomes, stock] =
    await Promise.all([
      getRepo(Child).count({ where: { tenantId } }),
      getRepo(Group).count({ where: { tenantId } }),
      getRepo(Staff).count({ where: { tenantId } }),
      getRepo(AttendanceRecord).count({ where: { tenantId } }),
      getRepo(StockMovement).count({ where: { tenantId } }),
      getRepo(Invoice).count({ where: { tenantId } }),
      getRepo(Payment).count({ where: { tenantId } }),
      getRepo(Expense)
      ...
      getRepo(StockItem)
        .createQueryBuilder('si')
        .select('COALESCE(SUM(si.totalValue), 0)', 'sum')
        .where('si.tenantId = :tenantId', { tenantId })
        .getRawOne<{ sum: string }>(),
    ]);

  const revenue = Number((incomes as { sum?: string } | undefined)?.sum ?? 0);
  const expense = Number((expenses as { sum?: string } | undefined)?.sum ?? 0);
  ...
  console.log('  Ombor qiymati:', mln(Number((stock as { sum?: string } | undefined)?.sum ?? 0)), "so'm");
